...
`EMAIL_REPLY_TO` is optional for outbound reports/alerts. `OWNER_EMAIL_INBOUND_ADDRESS` is the address used as the reply-to for bot replies so the owner can continue an email thread with SignalHost. `EMAIL_PROVIDER` is also optional when `RESEND_API_KEY` is present, but setting it makes the configuration obvious.
...
Do not enable receiving on the root `signalhost.ai` domain because Gmail/Google Workspace handles human inboxes there. Use a separate subdomain for the agent, such as `inbound.signalhost.ai`.

1. In Resend, add `inbound.signalhost.ai` as a receiving domain.
...
7. Set `OWNER_EMAIL_INBOUND_ADDRESS` to the address owners should reply to, for example:

```text
updates@inbound.signalhost.ai
```
...
Inbound owner emails only run commands when the sender matches a trusted `business_contacts.email` with owner-assistant permissions enabled.
