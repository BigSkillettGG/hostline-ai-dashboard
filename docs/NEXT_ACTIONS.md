...
- Preserve the verified commercial hierarchy while closing executable role and write coverage before broader department ownership.
...
- The original scope-switching verification was green at 97 test files / 590 tests, TypeScript, lint with zero errors and eight pre-existing warnings, all three production builds, a passing six-tenant production RLS gate, and a real Supabase customer browser login showing only its own location. The controlled partner identities and browser coverage described below close the remaining partner-switching dependency.
- Migration `20260806170000_commercial_function_privilege_hardening.sql` is applied and verified in production. Lovable recorded it under generated version `20260806173859`; keep that generated file as a no-op ledger marker and the full DDL in canonical version `20260806170000`. Live inspection confirmed anon denial on all 49 covered helpers, authenticated execution only on the 20 direct RLS predicates, and service-role execution on all 49.
- The repeatable `npm run check:commercial-write-isolation` production gate now passes for all six customer identities: 48 cross-tenant PATCH probes returned no writable rows across partner administration, organizations, locations, departments, queues, phone numbers, number routes, and telephony accounts. It uses only current-value updates and issues no inserts/deletes. Partner-role coverage is recorded below; controlled insert/delete coverage remains open.
- Department navigation and partner/customer switching are published at `signalhost.ai`. The live bundle exposes partner workspaces and default departments and no longer contains `second location (soon)`. Existing calls/requests/workflows/knowledge/reports remain location-scoped until their department ownership is deliberately modeled.
- Production voice health now calls Vapi the preferred managed runtime. `/health` remains production-ready, reports `preferredProvider: vapi`, keeps direct OpenAI Realtime SIP as maintained fallback, and keeps LiveKit quarantined; the `vapi_pilot` readiness ID remains a compatibility identifier.
- Controlled partner identities are now provisioned. `npm run check:commercial-partner-scope` passes with six direct-partner customer organizations, zero isolation-partner customer resources, positive partner predicates, and nine denied cross-partner current-value PATCH probes. Direct-partner browser switching from Olive & Ember to Summit Air is verified.
- The isolation browser check exposed authenticated fallback to local Olive & Ember demo data when a partner has no visible location. Commit `563bc4e` now renders `No customer workspaces assigned` before the app shell in that state and is published at `signalhost.ai`. The production control login shows no app shell, customer identity, metrics, or demo data. All three commercial production gates and voice health passed afterward; the partner-scope slice is complete.
- Organization and department role verification is complete. `npm run check:commercial-role-matrix` passes for owner, admin, manager, staff, department manager, department agent, and department viewer against the isolated `SignalHost Authorization QA` production hierarchy. It verifies membership and row visibility, public access/manage/operate predicates, positive and negative current-value writes, restricted-department behavior, operational request writes, and cross-partner denial. The fixture has no phone, route, agent, transfer, call, telephony-account, or SIP-trunk resources. Repository verification is green at 101 test files / 615 tests, TypeScript, lint with zero errors and eight pre-existing warnings, the production dashboard build, all four commercial production gates, voice health, and whitespace validation. See `docs/COMMERCIAL_ROLE_MATRIX_VERIFICATION.md`.
- Next foundation slice: add controlled insert/delete and provisioning-trigger coverage using disposable fixtures or rollback-capable tests, then add immutable, attributable support-session audit before broader department ownership. Preserve Vapi routing and do not mutate customer business rows.
