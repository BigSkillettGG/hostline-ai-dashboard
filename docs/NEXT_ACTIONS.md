...
- The repeatable `npm run check:commercial-write-isolation` production gate now passes for all six customer identities: 48 cross-tenant PATCH probes returned no writable rows across partner administration, organizations, locations, departments, queues, phone numbers, number routes, and telephony accounts. It uses only current-value updates and issues no inserts/deletes. The repository is green at 99 test files / 599 tests, TypeScript, lint with zero errors and eight pre-existing warnings, the read-only production gate, and whitespace validation. Partner-role, department-role, and disposable-fixture insert/delete coverage remain open.
...
- Next foundation slice: define and verify organization-role and department-role positive/negative matrices before broadening department ownership into calls, requests, workflows, knowledge, or reports. Keep inserts/deletes limited to disposable fixtures or rollback-capable tests and preserve Vapi routing.
