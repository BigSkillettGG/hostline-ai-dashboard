...
- Repository migration `20260806170000_commercial_function_privilege_hardening.sql` now closes the default PostgreSQL `PUBLIC EXECUTE` exposure for commercial `SECURITY DEFINER` helpers. Its repository checks pass at 98 test files / 595 tests, TypeScript, lint with zero errors and eight pre-existing warnings, and whitespace validation. Apply it next, verify anonymous/authenticated denial of the internal default-account RPC, rerun the six-tenant checker, and confirm service-trigger provisioning compatibility before marking it live.
...
- Department navigation context is now repository-complete beneath the active location: the header reads only RLS-visible active departments, defaults deterministically, and clears stale selections when the location changes. Verification passes at 99 test files / 602 tests, TypeScript, lint with zero errors and eight pre-existing warnings, the production dashboard build, and whitespace validation. Do not describe existing calls/requests/workflows/knowledge/reports as department-filtered yet; their storage and query contracts remain location-scoped. Public bundle publication and browser verification are pending.
- Repository voice-health metadata now calls Vapi the preferred managed runtime instead of an optional quarantined pilot. Verification passes at 99 test files / 603 tests, TypeScript, lint with zero errors and eight pre-existing warnings, the voice production build, and whitespace validation. Deploy the voice service and confirm `/health` no longer exposes the obsolete description; the `vapi_pilot` readiness ID remains a compatibility identifier.
- Public dashboard deployment is still pending: `https://signalhost.ai` continues to serve the old `second location (soon)` bundle even though `165066d` is on `origin/main`.
