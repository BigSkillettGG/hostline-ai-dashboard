...
- The isolation browser check exposed authenticated fallback to local Olive & Ember demo data when a partner has no visible location. The repository now renders `No customer workspaces assigned` before the app shell in that state. Publish and retest this guard next; do not mark the partner-scope slice complete until the production control login no longer displays demo data.
