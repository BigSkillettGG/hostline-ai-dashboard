...
The repository-only telephony ownership foundation in `docs/COMMERCIAL_TELEPHONY_FOUNDATION.md` and migration `20260806070000_commercial_telephony_ownership_foundation.sql` extends the control plane with non-secret carrier/runtime/PBX account ownership, dormant SIP trunk identities, and observed number-to-default-department routes. `phone_numbers` remains the live compatibility record, the global Twilio SIP trunk remains environment-managed, and neither the dashboard nor voice runtimes read the new route table. Production application and live backfill/RLS verification are still pending.
