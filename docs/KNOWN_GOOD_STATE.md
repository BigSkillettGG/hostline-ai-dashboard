...
- Verification for that slice was green at 97 test files / 590 tests, TypeScript, lint with zero errors and eight pre-existing warnings, all three production builds, the authenticated production RLS verifier, and a real Supabase customer browser login. The later controlled partner verification below closes the original partner-browser dependency.

Controlled partner verification is now provisioned in production. The `SignalHost Direct` QA owner receives no customer membership yet sees all six customer organizations through partner RLS; the inactive isolation-control partner owner sees zero customer resources. `npm run check:commercial-partner-scope` passes positive partner/organization predicates and nine current-value cross-partner PATCH denials across the established commercial boundaries. Browser verification switched the direct partner from Olive & Ember to Summit Air and retained the correct default department and vertical data. The control browser check exposed a local-demo fallback for authenticated users with no visible location; commit `563bc4e` replaces it with an explicit unassigned-workspace state and is published at `signalhost.ai`. The production control login now shows no app shell, customer identity, metrics, or demo data. All three commercial gates and the voice-health gate passed after publication; Vapi remains preferred.
...
The customer-to-customer production write-isolation gate is also passing. `npm run check:commercial-write-isolation` authenticated as all six demo customers and denied 48 current-value PATCH probes across partner administration, organizations, locations, departments, queues, phone numbers, number routes, and telephony accounts. No probe returned a writable row, no insert/delete was issued, and no business data changed. Partner-role coverage is recorded above; controlled insert/delete coverage remains open.

The organization/department role-matrix gate is now passing against the isolated `SignalHost Authorization QA` production hierarchy. `npm run check:commercial-role-matrix` authenticated as owner, admin, manager, staff, department manager, department agent, and department viewer. All membership visibility, inherited/restricted department and queue visibility, public access/manage/operate predicates, positive/negative current-value writes, operational request writes, and cross-partner denials matched the documented matrix. The fixture is inactive/internal and has no phone numbers, routes, agent configurations, transfer targets, calls, telephony accounts, or SIP trunks. No insert/delete was issued and no live customer or voice configuration changed. Repository verification is green at 101 test files / 615 tests, TypeScript, lint with zero errors and eight pre-existing warnings, the production dashboard build, all four commercial production gates, voice health, and whitespace validation. `docs/COMMERCIAL_ROLE_MATRIX_VERIFICATION.md` is the durable fixture and capability contract.

The repository now carries explicit department navigation context below the active location. The header loads only RLS-visible active departments, keeps a valid selection or chooses the location default, and clears stale department state on location switches. This is navigation context only: existing calls, requests, workflows, knowledge, and reports remain location-scoped until their department ownership is deliberately modeled. The dashboard through commit `d6c9dc179e70ba6259820dc452d34a951c5c3628` was published to `signalhost.ai` on 2026-08-06. The live bundle exposes partner and department workspace controls and no longer contains `second location (soon)`; both production isolation gates passed after publication. Controlled partner browser verification is recorded above.
