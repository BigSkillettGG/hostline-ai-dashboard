...
This service is the production path for inbound SignalHost calls, chats, owner commands, and operational webhooks. Vapi is the preferred production voice runtime. Direct OpenAI Realtime SIP is the maintained fallback, and Twilio ConversationRelay remains a legacy fallback/test path.
...
- Supports the preferred Vapi webhook bridge, the maintained direct OpenAI Realtime SIP fallback, and the legacy ConversationRelay path.
...
- Exposes a canonical provider catalog in `GET /health` and `GET /ready`; the catalog is observational and does not reroute calls.
...
- Accepts provider-neutral owner email command webhooks at `POST /owner/email-command`, plus signed Resend inbound email webhooks at `POST /resend/inbound-email`, matching trusted owner/manager email addresses before applying live updates, report questions, or permanent knowledge updates. Per-business aliases such as `ava-olive-and-ember+<location-id>@agents.signalhost.ai` route to the right location through the plus-addressed location id.
...
Preferred Vapi runtime:

- `VAPI_API_KEY`, `VAPI_WEBHOOK_SECRET`, `VAPI_PILOT_ENABLED=true`, and the intended location allow-list.
- Fixed Vapi assistants attached to the known-good demo numbers; see `docs/vapi-pilot.md`.
- SignalHost `/vapi/webhook` configured for provider events and tool calls.

Maintained direct-SIP and legacy Twilio capabilities:
...
- Optional inbound owner email with `RESEND_WEBHOOK_SECRET` and `OWNER_EMAIL_INBOUND_ADDRESS` after configuring a receiving subdomain such as `agents.signalhost.ai`.
...
- `GET /email/readiness?locationId=...` returns the safe Resend activation checklist, inbound webhook URL, receiving domain, fallback address, and outbound sender status. It does not expose secrets.
...
- `POST /resend/inbound-email` accepts signed Resend `email.received` webhooks, fetches the full received email from Resend, routes it through owner email commands, and replies to the trusted sender when direct email delivery is configured. If the recipient contains a location id, the command is scoped to that business even when the same owner manages multiple locations.
...
- `POST /twilio/recording-status` receives Twilio recording callbacks. The OpenAI SIP path auto-starts Twilio call recording when the incoming webhook includes a Twilio `CallSid`; otherwise configure Twilio call/SIP trunk recording to send completed recording events to `https://your-voice-service/twilio/recording-status`. The service stores the MP3 URL on `calls.recording_url`.
