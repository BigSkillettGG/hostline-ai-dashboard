...
const envSchema = z.object({
  ...
  OPENAI_REALTIME_SERVER_VAD_THRESHOLD: z.coerce.number().min(0.05).max(0.98).default(0.93),
  OPENAI_REALTIME_SERVER_VAD_SILENCE_MS: z.coerce.number().int().min(200).max(2200).default(1100),
  ...
  OPENAI_REALTIME_GREETING_DELAY_MS: z.coerce.number().int().min(0).max(2500).default(900),
  OPENAI_REALTIME_DETAIL_CAPTURE_RESPONSE_DELAY_MS: z.coerce.number().int().min(0).max(2000).optional(),
  OPENAI_REALTIME_MANUAL_RESPONSE_DELAY_MS: z.coerce.number().int().min(0).max(2000).optional(),
